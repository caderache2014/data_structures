...
class AVLTree(object):
    ...
    def insert(self, value, balance=False):
        ...
        while True:
            ...
            if balance:
                #Rebalance the tree following each insertion
                path.pop()
                if path:
                    #grandparent of inserted node
                    node = path.pop()
                else:
                    return
                while node:
            ...
            def _depth(self, current_node, local_root):
                left_depth = 0
                right_depth = 0
                if local_root.left:
            ...
            def _avl(self, node):
                ...
                if current_node_balance == 2:
                    if self._balance(node.left) == -1: #LR case
                        node.left = self._rotate_left(node.left) # becomes a LL case
                    return self._rotate_right(node)
                elif current_node_balance == -2:
                    if self._balance(node.right) == 1: #RL class 
                        node.right = self._rotate_right(node.right) #reduce to a RR case
                    return self_rotate_left(node)
                else:
                    return node
            ...
            def _pre_order(self, node):
                if node: 
                    yield node.value
                    for left_subtree_value in self._pre_order(node.left):
                        yield left_subtree_value
                    for right_subtree_value in self._pre_order(node.right):
            ...
            def _post_order(self, node):
                if node:
                    for left_subtree_value in self._post_order(node.left):
                        yield left_subtree_value
                    for right_subtree_value in self._post_order(node.right):
                        yield right_subtree_value
                    yield node.value
            ...
            def _delete(self, val, node, balance=False):
                ...
                if node.value == val:
                    ...
                    if node.left:
                        node.left = self_delete(val, node.left, balance)
                    return_node = node
